...
M.foldText = function()
  ...
  local parsed_line = require('utils.misc').parse_line(vim.v.foldstart)
  if not parsed_line then
    return "ERR >>>"
  end
  local suffix = (' ' ..  fold_signs[6] .. ' %d'):format(vim.v.foldend - vim.v.foldstart)
  table.insert(parsed_line, {suffix, 'GruvboxAquaBold'})
  return parsed_line
end
